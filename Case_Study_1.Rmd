...
#### The pie chart below shows the number of NA values 
```{r}
...
#### Summary statistics for Alcoholic content. 
```{r, message= FALSE}
...
#### The scatter plot below shows the relationship between alcohol content (ABV) in each beer and bitterness. As you can see the plot shows that the higher alcohol content the more likely the beer is to have a higher bitterness (IBU)
```{r}
#Create an new dataframe ABV_IBU that consists of Beer ID, ABV, IBU, Beer Style, Ounces
ABV_IBU <- data.frame(beers_brew$Beer_ID,beers_brew$ABV, beers_brew$IBU, beers_brew$Beer_Style, beers_brew$Ounces)

#rename the columns of the new data frame to "Beer_ID", "ABV", "IBU"
names(ABV_IBU) <- c("Beer_ID", "ABV", "IBU", "Beer_Style","Ounces")

#Create a scatter plot 
p3 <- ggplot(ABV_IBU, aes(ABV,IBU, color = IBU)) + geom_point(aes(size = Ounces)) + labs(title = "Alchol Content vs Bitterness", x = "Alcohol Content (ABV)", y = "Bitterness (IBU)")
