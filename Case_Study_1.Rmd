...
p<-ggplot(MedianABV ,aes(reorder(MedianABV$State,-Median_ABV),Median_ABV, fill= MedianABV$State)) + geom_bar(stat='identity')+ theme(text=element_text(size=10),plot.title = element_text(hjust = 0.5),legend.position="none", axis.text.x = element_text(angle = 90))+ ggtitle("Median ABV Per-State") + labs(x = "State", y = "ABV")+ scale_color_gradient(low = "blue", high = "green")

p
```

#### The following map shows the States with the heighest median ABV
```{r}
#Load state name csv
...
#state_brew_ABV_map1<-state_brew_ABV_map + scale_fill_gradientn(colours = rev(rainbow(7))), guides(fill=TRUE))
